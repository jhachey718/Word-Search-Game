...
class WordSearch:
    ...
    def generate_words(self, amount_to_generate):
        if len(self.WORDS) == 0:
            ...
            word_file = Path('..') / 'word_lists' / 'words.txt'
            ...
            while not is_placed:
                ...
                if direction == 0 and col - len(word) >= 0 and row - len(word) >= 0:
                    for i in range(len(word)):
                        ...
                        new_row = row - i
                        # print(f"Placed {word}")
                        is_placed = self.__test_place(
                            word, temp_board, new_row, new_col, i)
                        if not is_placed:
                            break
